...
int PlayOneTurn(string playerName, ifstream &fin, ofstream &fout)
{
    int dieRoll;
    
    fout << playerName << "'s turn \n";
    
    PressEnterToRoll(fin, fout);
    
    dieRoll = RollDie();
    
    fout << playerName << " rolls: \n";
    DisplayDie(dieRoll, fout);
